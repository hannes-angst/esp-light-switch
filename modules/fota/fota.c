...
void ICACHE_FLASH_ATTR start_ota(char *url) {
	...
	str[i] = '/';
	p--;
	str = p; // /api/sensor1/1.2.3/user1.bin

	i = os_strlen(str);
	if (system_upgrade_userbin_check() == UPGRADE_FW_BIN1)
		str[i - 5] = '2';                 // /api/sensor1/user2.bin
	if (system_upgrade_userbin_check() == UPGRADE_FW_BIN2)
		str[i - 5] = '1';                 // /api/sensor1/user1.bin
	ota_client->url = (char*) os_zalloc(os_strlen(str) + 1);
	os_strncpy(ota_client->url, str, os_strlen(str));

	INFO("Binary host: %s, port %d, path: %s\r\n", ota_client->host, ota_client->port, ota_client->url);
